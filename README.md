...
<h4>model.py</h4> Defines the overall NeuMTL model architecture and the forward propagation workflow. <h4>gated_attention.py</h4> Implements the gated attention mechanism for modality-aware feature weighting. <h4>shared_attention.py</h4> Implements the shared attention layer for representation learning across tasks. <h4>Task_Specific_Attention.py</h4> Implements task-specific attention modules to capture unique task-level feature contributions. <h4>gra.py</h4> Multi-task gradient regulation strategies, such as Gradient Alignment, GRL, or PCGrad. <h4>training.py</h4> Controls the full training pipeline: `DataLoader → Model → Optimizer → Evaluation → Model Saving`. <h4>utils.py</h4> Contains utility functions including loss computation, evaluation metrics, logging, and checkpoint saving. <h4>create_data.py</h4> Data preprocessing script (version 1) to construct training and validation datasets. <h4>create_data2.py</h4> Data preprocessing script (version 2) for alternate input formats or augmentation strategies. <h4>data.tar.gz</h4> Compressed raw dataset (version 1). <h4>data2.tar.gz</h4> Compressed raw dataset (version 2, possibly including preprocessed or augmented data). <h4>framework.jpg</h4> Illustration of the NeuMTL multimodal multi-task framework. <h4>__pycache__</h4> Python cache files (can be ignored).
